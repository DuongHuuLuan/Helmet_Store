...
from app.infrastructure.database.models import Product, Category
...
class ChatbotCatalogService:
    _STOPWORDS = {
        "a", "an", "and", "ao", "ban", "bao", "bên", "cho", "co", "con",
        "cua", "cần", "duoc", "gia", "giá", "giup", "giúp", "hang", "hãy",
        "hay", "hien", "hiện", "khong", "không", "loai", "loại", "mai",
        "minh", "mình", "mu", "mũ", "muon", "muốn", "nao", "nào", "nay",
        "này", "nen", "nên", "non", "nón", "san", "shop", "sp", "toi",
        "tôi", "tu", "tư", "van", "ve", "về", "voi", "với",
    }
    ...
    def _build_variants(db: Session, product: Product, price_cap: Optional[int]) -> List[Dict[str, Any]]:
        ...
        for detail in getattr(product, "product_details", []) or []:
            ...
            stock = WarehouseRepositoryImpl(db).get_total_stock_for_detail(detail)
            variants.append({
    ...
    def search_products(
        ...
        prod_repo = ProductRepositoryImpl(db)
        max_products = max(1, limit or settings.CHATBOT_MAX_PRODUCTS)
        base_page = prod_repo.get_all_with_details(
            page=1,
            per_page=None,
            keyword=None,
            category_id=None,
        ...
        product_cards.sort(key=lambda item: (
            -item["score"],
            item["price"] if isinstance(item["price"], int) else 10**12,
            -item["product_id"],
        ))
