import 'package:b2205946_duonghuuluan_luanvan/presentation/helmet_designer/cubit/helmet_designer_state.dart';
...
class HelmetDesignerCubit extends Cubit<HelmetDesignerState> {
  final GetStickerCatalogUseCase _getStickerCatalog;
  final GetDesignDetailUseCase _getDesignDetail;
  final GenerateAiStickerUseCase _generateAiSticker;
  final SaveDesignUseCase _saveDesign;
  final CreateShareLinkUseCase _createShareLink;
  final TranscribeAiStickerVoiceUseCase _transcribeAiStickerVoice;
  final OrderDesignUseCase _orderDesign;
  final GetProductDetailUseCase _getProductDetail;

  HelmetDesignerCubit(
    this._getStickerCatalog,
    this._getDesignDetail,
    this._generateAiSticker,
    this._saveDesign,
    this._createShareLink,
    this._transcribeAiStickerVoice,
    this._orderDesign,
    this._getProductDetail,
  ) : super(HelmetDesignerState(
          currentDesign: HelmetDesign(
            id: 0,
            helmetProductId: 0,
  ...
  List<StickerLayer> get visibleStickerLayers {
    ...
    return layers.where((layer) => (layer.viewImageKey ?? '').trim() == activeKey).toList();
  }
  ...
  ProductImage? get activeDesignView {
    ...
    for (final item in _designViews) {
      if ((item.viewImageKey ?? '').trim() == activeKey) return item;
    }
  ...
  void _emitState() {
    emit(HelmetDesignerState(
      stickerCatalog: List.unmodifiable(_stickerCatalog),
      stickerLayers: List.unmodifiable(_stickerLayers),
      designViews: List.unmodifiable(_designViews),
      currentDesign: _currentDesign,
      isLoadingCatalog: isLoadingCatalog,
      isLoadingDesign: isLoadingDesign,
      isTranscribingSticker: isTranscribingSticker,
      isGeneratingSticker: isGeneratingSticker,
      isSavingDesign: isSavingDesign,
      isSharingDesign: isSharingDesign,
      isOrderingDesign: isOrderingDesign,
  ...
  Future<void> loadStickerCatalog() async {
    ...
    errorMessage = null;
    _emitState();
    final result = await _getStickerCatalog();
    result.fold(
      (failure) {
  ...
  void startNewDesign({
    ...
    _selectedProductDetailId =
        productDetailId != null && productDetailId > 0 ? productDetailId : null;
  ...
  Future<void> loadDesign(int designId) async {
    ...
    errorMessage = null;
    _emitState();
    final result = await _getDesignDetail(designId);
    await result.fold(
      (failure) async {
        errorMessage = failure.message;
      },
  ...
  Future<StickerTemplate?> generateAiSticker(
    ...
    errorMessage = null;
    _emitState();
    final result = await _generateAiSticker(request);
    return result.fold(
      (failure) {
        errorMessage = failure.message;
  ...
  Future<String?> transcribeAiStickerVoice(String audioPath) async {
    ...
    errorMessage = null;
    _emitState();
    final result = await _transcribeAiStickerVoice(audioPath);
    return result.fold(
      (failure) {
        errorMessage = failure.message;
  ...
  void bringSelectedLayerForward() => _moveSelectedLayerBy(1);
  void sendSelectedLayerBackward() => _moveSelectedLayerBy(-1);
  void bringSelectedLayerToFront() => _moveSelectedLayerTo(_sortedLayers().length - 1);
  void sendSelectedLayerToBack() => _moveSelectedLayerTo(0);
  ...
  void removeSelectedLayer() {
    ...
    _normalizeLayerOrder(notify: false);
    selectedLayerId =
        _stickerLayers.isNotEmpty ? _sortedLayers().last.id : null;
    _syncCurrentDesign();
    _emitState();
  }
  ...
  Future<HelmetDesign?> saveCurrentDesign() async {
    ...
    final result = await _saveDesign(_buildCurrentDesign());
    return result.fold(
      (failure) {
        errorMessage = failure.message;
  ...
  Future<String?> shareCurrentDesign() async {
    ...
    final result = await _createShareLink(_currentDesign.id);
    return result.fold(
      (failure) {
        errorMessage = failure.message;
        isSharingDesign = false;
  ...
  Future<bool> orderCurrentDesign() async {
    ...
    final result = await _orderDesign(
      _currentDesign.id,
  ...
  List<ProductImage> _sortedDesignViews(List<ProductImage> items) {
    ...
    views.sort((left, right) {
      final priority = viewImageKeyPriority(left.viewImageKey)
  ...
  String? _resolveLoadedViewImageKey({
    ...
    final byImage =
        _resolveInitialViewImageKey(fallbackImageUrl: fallbackImageUrl);
    if ((byImage ?? '').isNotEmpty) return byImage;
    for (final sticker in stickers) {
      final key = (sticker.viewImageKey ?? '').trim();
      if (key.isEmpty) continue;
      final exists = _designViews
          .any((item) => (item.viewImageKey ?? '').trim() == key);
  ...
  Future<HelmetDesign> _restoreDesignContext(HelmetDesign design) async {
    ...
    return productResult.fold(
      ...
      (product) {
        final resolvedViews =
            product.resolveDesignViewsForBaseImage(design.helmetBaseImageUrl);
        ...
        return design.copyWith(
          helmetName:
              design.helmetName.isNotEmpty ? design.helmetName : product.name,
  ...
  void _setActiveViewImageKey(String? viewImageKey, {bool notify = true}) {
    ...
    final matched = _designViews
        .where((item) => (item.viewImageKey ?? '').trim() == normalized);
  ...
  int? _resolveInitialSelectedLayerId() {
    ...
    if (visible.isNotEmpty) return visible.last.id;
    final ordered = _sortedLayers();
    if (ordered.isNotEmpty) return ordered.last.id;
    return null;
  }
  ...
  void _upsertStickerTemplate(StickerTemplate template) {
    final index =
        _stickerCatalog.indexWhere((item) => item.id == template.id);
    if (index >= 0) {
  ...
  double _unitValue(double value) => value.clamp(0.0, 1.0).toDouble();
  double _scaleValue(double value) => value.clamp(0.1, 4.0).toDouble();
}
